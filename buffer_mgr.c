    ...
    for (i = 0; i < bufferSize; i++)
    {
        if (pageFrame[frontIndex].fixCount == 0)
        {
            if (pageFrame[frontIndex].dirtyBit == 1)
            {
                ...
                writeFileBlock(pageFrame[frontIndex].pageNum, &fh, pageFrame[frontIndex].data);
                writeCount++;
            }
...
extern void LFU(BM_BufferPool *const bm, PageFrame *page)
{
    ...
    if (pageFrame[leastFreqIndex].dirtyBit == 1)
    {
        SM_FileHandle fh;
        openPageFile(bm->pageFile, &fh);
        writeFileBlock(pageFrame[leastFreqIndex].pageNum, &fh, pageFrame[leastFreqIndex].data);
        writeCount++;
    }
...
extern void LRU(BM_BufferPool *const bm, PageFrame *page)
{
    ...
    if (pageFrame[leastHitIndex].dirtyBit == 1)
    {
        SM_FileHandle fh;
        openPageFile(bm->pageFile, &fh);
        writeFileBlock(pageFrame[leastHitIndex].pageNum, &fh, pageFrame[leastHitIndex].data);
        writeCount++;
    }
    ...
    while (1)
    {
        ...
        if (pageFrame[clockPointer].hitNum == 0)
        {
            if (pageFrame[clockPointer].dirtyBit == 1)
            {
                SM_FileHandle fh;
                openPageFile(bm->pageFile, &fh);
                writeFileBlock(pageFrame[clockPointer].pageNum, &fh, pageFrame[clockPointer].data);
                writeCount++;
            }
...
extern RC forceFlushPool(BM_BufferPool *const bm)
{
    ...
    for(i = 0; i < bufferSize; i++)
    {
        if(pageFrame[i].fixCount == 0 && pageFrame[i].dirtyBit == 1)
        {
            SM_FileHandle fh;
            openPageFile(bm->pageFile, &fh);
            writeFileBlock(pageFrame[i].pageNum, &fh, pageFrame[i].data);
...
extern RC markDirty(BM_BufferPool *const bm, BM_PageHandle *const page)
{
    ...
    return RC_PAGE_NOT_FOUND_IN_BUFFER;
}
